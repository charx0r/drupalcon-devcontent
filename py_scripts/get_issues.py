...
"""
    get_issue
    ~~~~~~~~~~~~~
    Takes in module name as an argument and then builds a database of issues, comments.
    run when we want to update specific module data
...
#get module machine name
machine_name = sys.argv[1]
print machine_name
#loop all modules
for module_data in module_collection.find({'field_project_machine_name':machine_name},{'nid':1}).sort([('nid',-1)]).limit(1000):
	current_pager = 0
	try:
